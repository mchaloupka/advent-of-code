...
pub fn run(input: &str) {
    ...
    let mut areas_by_size: Vec<_> = red_points.iter().enumerate().flat_map(|(i, point)| {
        red_points.iter().skip(i + 1).map(|other_point| {
            let area =
    ...
    let polygon: Polygon<_> = Polygon::new(
        geo::LineString::from(line_string_points),
        vec![],
    );
